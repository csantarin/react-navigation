...
exports.renderCompatNavigatorScreens = renderCompatNavigatorScreens;
...
function renderCompatNavigatorScreens(args) {
  const {
    routeConfig,
    routeNames,
    navigatorProps,
  ...
  return routeNames.map(name => {
    ...
    if ('getScreen' in routeConfigItem) {
      ...
      getScreenComponent = () => routeConfigItem;
    } // To prevent infinite render loops caused by getScreenComponent being redefined when navigation happens.


    const getScreenComponentRef = React.useRef(getScreenComponent);

    const screenOptions = ({
      navigation,
      route
    }) => {
      const routeNavigationOptions = 'navigationOptions' in routeConfigItem ? routeConfigItem.navigationOptions : undefined;
      const screenNavigationOptions = getScreenComponent().navigationOptions;
    ...
    return /*#__PURE__*/React.createElement(Pair.Screen, {
      ...
      getComponent: getScreenComponentRef.current
    }));
  });
}
